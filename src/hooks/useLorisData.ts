...
export const useLorisData = (isPremium: boolean = false, refreshInterval = 60000) => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      let processed = processApiData(apiData);
      
      // Apply restriction: only top 5 for non-premium users
      if (!isPremium) {
      ...
      let processed = processApiData(MOCK_DATA);
